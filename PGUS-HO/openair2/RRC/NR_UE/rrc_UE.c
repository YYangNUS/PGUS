...
static void nr_rrc_ue_process_rrcReconfiguration(NR_UE_RRC_INST_t *rrc, int gNB_index, NR_RRCReconfiguration_t *reconfiguration)
{
  ...
  switch (reconfiguration->criticalExtensions.present) {
    case NR_RRCReconfiguration__criticalExtensions_PR_rrcReconfiguration: {
      ...
      LOG_A(NR_RRC, "nr_rrc_ue_process_rrcReconfiguration Start time %lld\n", milliseconds);
      ...
      xer_fprint(stdout, &asn_DEF_NR_RRCReconfiguration_IEs, (const void *)ie);

      LOG_E(NR_RRC, "RRCReconfiguration includes Measurement Configuration\n");
      LOG_E(NR_RRC, "RRCReconfiguration includes Measurement Configuration\n");
      LOG_E(NR_RRC, "RRCReconfiguration includes Measurement Configuration\n");

      /****************************** PGUS Extention ******************************/
      const char *env_activate = "~/jlab_env/bin/activate";
      const char *script_dir = "path_to_SBS/";
      const char *script_name = "ue.py";

      char command[512];

      snprintf(command, sizeof(command), 
                "bash -c 'source %s && cd %s && python %s'",
                env_activate, script_dir, script_name);

      int result = system(command);

      if (result == -1) {
          LOG_E(NR_RRC, "Python script executed with exit status %d\n", result);
      } else {
          LOG_A(NR_RRC, "Python script executed with exit status %d\n", result);
      }
      ...
      long long milliseconds2 = (long long)tv2.tv_sec * 1000LL + (long long)(tv2.tv_usec / 1000LL);
      LOG_A(NR_RRC, "nr_rrc_ue_process_rrcReconfiguration End time %lld\n", milliseconds2);
